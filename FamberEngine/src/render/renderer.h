...
static const char* R_VERT =
    ...
    "void main(){ vUV=aUV; vLM=aLM; gl_Position=uMVP*vec4(aPos,1.0); }\n";

static const char* R_FRAG =
    "#version 120\n"
    "uniform sampler2D uBase; uniform sampler2D uLight; uniform float uScale;\n"
    "varying vec2 vUV; varying vec2 vLM;\n"
    "void main(){\n"
    "  vec3 b=texture2D(uBase,vUV).rgb;\n"
    "  vec3 l=texture2D(uLight,vLM).rgb;\n"
    "  vec3 c=b*l*uScale;\n"
    "  gl_FragColor=vec4(pow(c, vec3(0.73)),1.0);\n" // mild gamma lift
    "}\n";

struct FaceDraw { int start, count; GLuint baseTex, lmTex; };

struct Renderer {
    GLuint prog = 0, vbo = 0, whiteTex = 0;
    GLint uMVP = -1, uBase = -1, uLight = -1, uScale = -1, aPos = -1, aUV = -1, aLM = -1;
    ...
    // ---- loaded BSP ----
    void initBSP(const Bsp& bsp) {
        setupProgram();
        scale = 1.0f; // GoldSrc lightmaps are ~1:1
        std::vector<GLuint> texIds;
        for (const BspTex& t : bsp.textures) texIds.push_back(uploadRGB(t.rgb.data(), t.w, t.h, GL_REPEAT));
        ...
        for (const BspFace& bf : bsp.faces) {
            ...
            faces.push_back({start, (int)bf.verts.size() / 7, baseTex, lmTex});
        }
        finishBuffer(verts);
    }
    ...
    void draw(const Vec3& eye, const Vec3& fwd, int w, int h) {
        ...
        Mat4 proj = mat4Perspective(90.0f * 3.14159265f / 180.0f, aspect, 4.0f, 16384.0f);
